...
export type IndicatorResult =
  | { def: IndicatorDef; ok: true; latest: Observation; previous: Observation }
  | { def: IndicatorDef; ok: false; error: string };
...
const OPEN_INDICATORS: IndicatorDef[] = [
  ...
  { id: "usdjpy", label: "ドル円", unit: "", decimals: 2, change: "abs", source: "ECB", load: () => fetchEcbRate("USD", "JPY") },
  { id: "jgb10y", label: "日本10年国債利回り", unit: "%", decimals: 3, change: "bp", source: "財務省", load: () => fetchJgbYield("10年") },
];
...
export async function loadIndicators(defs: IndicatorDef[], fredApiKey: string | undefined): Promise<IndicatorResult[]> {
  const fredCache = new Map<string, Promise<Observation[]>>();
  const ctx: LoadContext = {
    fred: (seriesId) => {
      if (!fredApiKey) return Promise.reject(new Error("FRED_API_KEY is not set"));
      let pending = fredCache.get(seriesId);
      if (!pending) {
        pending = fetchFredObservations(seriesId, fredApiKey);
  ...
  const settled = await Promise.allSettled(defs.map((def) => def.load(ctx)));
  return settled.map((s, i): IndicatorResult => {
    const def = defs[i];
    if (s.status === "rejected") return { def, ok: false, error: errorMessage(s.reason) };
    const [latest, previous] = s.value;
    if (!latest || !previous) return { def, ok: false, error: `${def.id}: not enough observations` };
    return { def, ok: true, latest, previous };
  });
}
